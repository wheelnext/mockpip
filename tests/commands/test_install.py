...
class TestMockpipMain(unittest.TestCase):
    ...
    def test_install_command_pypi(self, index_url):
        ...
        assert (
            f"Received install request for: `requests` from index: {index_url}"
            in result.stdout
        )
        assert (
            f"Querying `{index_url.rstrip('/')}/requests/` for package `requests`"
            in result.stdout
        )
        assert (
            f"Successfully fetched package data from `{index_url.rstrip('/')}/requests/"
            in result.stdout
        )
    ...
    def test_install_nonexisting_pkg(self):
        ...
        assert (
            f"Received install request for: `{pkg_name}` from index: https://pypi.org/simple"
            in result.stdout
        )  # noqa: E501
        assert (
            f"Querying `https://pypi.org/simple/{pkg_name}/` for package `{pkg_name}`"
            in result.stdout
        )  # noqa: E501
        assert (
            f"No candidate found for `{pkg_name}` from `https://pypi.org/simple/{pkg_name}/`"
            in result.stdout
        )  # noqa: E501
